...
function messageRusltFix() {
    ...
    myArray.forEach(function (element) {
        messageRuslt.innerHTML += `<p style="color: #79acf1;font-size: 25px;position: relative;right: 45px;top: 50px">${element}</p>`;
    });
...
function chekFileFletSupdes() {
    ...
    if (
        ...
        (supplierSubnetNumber(numSupplierSubnetNumber, numMessage) === true &&
            compareStringsIgnoreCaseAndSpace(isValidNumSupplierSubnetNumber, numSupplier) ||
            supplierSubnetNumber(numSupplierSubnetNumber, numMessage) === false &&
            compareStringsIgnoreCaseAndSpace(isValidNumSupplierSubnetNumber, numSupplierSubnetNumber)
        )

        &&

        (supplierSubnetNumber(numBranchRetailer, isValidNumBranchRetailer) === true ||
            supplierSubnetNumber(numBranchRetailer, isValidNumBranchRetailer) === false &&
    ...
    if (supplierSubnetNumber(numBranchRetailer, isValidNumBranchRetailer) === false && !compareStringsIgnoreCaseAndSpace(isValidNumBranchRetailer, numBranchRetailer)) {
